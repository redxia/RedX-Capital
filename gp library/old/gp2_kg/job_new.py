...
'''
NOTE - Removing reference to gp.singleton in the code per the following observation:
*This option is very useful if you have scripts executed by crontab at small amounts of time.*
- Arguably not required when code is isolated within containers
'''
# import gp.singleton
'''
NOTE - Changing from gp.email to gp.gp_email to avoid naming conflicts
'''
import gp.gp_email
import gp.runlog
import gp.settings
...
import gp.gp_snowflake as snowflake
...
'''
NOTE -- Modified to work in Docker container
Long-Term: 
    - Add git + repository into container (may not be necessary)
    - Replace git_hash with random hash
'''

### HARD CODED unitl long term fix is applied
git_hash        = 'aa1a99f77fe0607d6c47ebfc4a5aea1b3b2dda7f'
git_status      = '?? README.md'
git_describe    = 'v20180326.2001-3231-gaa1a99f7'
git_timestamp   = '2020-12-10 09:29:17 -0700'
...
'''
NOTE -- Updating Snowflake + Log output locations here
- Going to import these variables into run_steps() to view outputs
...
__SNOWFLAKE_INTERNAL = f'@rawdata.s3.internal/job{__PARENT_FOLDER}/{__JOB_FOLDER}/{__RUN_DATE}.{run_id}.{__settings_name}.{git_hash}'
__S3_INTERNAL =  f's3://qsight.dev.logs/job{__PARENT_FOLDER}/{__JOB_FOLDER}/{__RUN_DATE}.{run_id}.{__settings_name}.{git_hash}'
...
def _finish(success=False, error_msg=None):
    ...
    '''
    NOTE - Remove print statements after dev
    '''
    ...
    try:
        ...
        if not gp.settings.get('email_enabled', default=False):
            ...
            gp.gp_email.send(subject,body=body, body_filename=None, plain=False, attachments=[])
...
def _uncaught_exception_handler(exc_type, value, traceback):
    ...
    '''
    TODO: Trace through exc_type argument
    - Commenting out elif statement -- removing gp.singleton
    '''
...
def _initialize():
    ...
    '''
    NOTE -- Got an error here:
    OSError: [Errno 6] No such device or address
    
    -- Replacing os.getlogin() with a lambda function that displays the current logged in user
    '''
...
######## Send output to s3 #########
'''
NOTE -- Adding print statements for testing -- remove for prod
'''
def send_job_output_to_s3(snowflake_archive =__SNOWFLAKE_INTERNAL, s3_archive = __S3_INTERNAL,
    ...
    print('SETTINGS FILE BASE: ', settings_file_base)
