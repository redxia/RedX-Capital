...
for ix, f in enumerate(fac_group_dedup):
    ...
    if (len(f[0]) == 1) and (len(f[1]) == 1):
        ...
        select count(*) as matches from
            (select date, mfg_catalog, sum(total_spend) as total_spend from temp.af.m_p_merge 
...
df_out.to_csv(r'C:\dev\da.research\personal\afontanella\MOA_PETREL_Overlap\overlap_facilities.csv')
            
            
df_details = pd.DataFrame(columns=['FACILITY_IDS_MOA', 'FACILITY_IDS_PETREL', 'MOA_FACILITY_OF_INTEREST', 'PETREL_FACILITY_OF_INTEREST',  'MATCHES_WITH', 'MATCHES_WITHOUT', 'DECISION'], data=details)         
