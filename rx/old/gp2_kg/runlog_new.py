...
def get_run_id():
    ...
    run_id = gp.gp_snowflake.fetchone('select rawdata.metadata.job_run_id.nextval as run_id')[0]
    assert isinstance(run_id, int)
    log.info(f'RUN_ID: {run_id}')

    # Set QUERY_TAG and CURRENT_RUN_ID
    gp.gp_snowflake.execute(f"alter session set query_tag='RUN_ID={run_id}'")
    gp.gp_snowflake.execute(f"set current_run_id={run_id}::number")
...
def log_run_start(job_group, job_name, settings_file, host_ip, host_name, windows_user, git_timestamp, git_hash, git_status, git_describe, details=None):
    ...
    warehouse = gp.gp_snowflake.get_warehouse_details()
    ...
    gp.gp_snowflake.execute(insert, args=(run_id, job_group, job_name, settings_file, host_ip, host_name, windows_user, git_timestamp, git_hash, git_status, git_describe, warehouse["size"], json.dumps(details))).fetchall()

    gp.gp_snowflake.execute('set current_git_hash=%s::text', git_hash).fetchall()
...
def log_run_finish(success, finish_error=None, details=None):
    ...
    insert = f'''
        ...
        where run_id=%s
    '''
    gp.gp_snowflake.execute(insert, args=(success, finish_error, json.dumps(details), run_id)).fetchall()

    # Delete old query history
    gp.gp_snowflake.execute('delete from rawdata.metadata.job_query_history where datediff(days, start_time, current_timestamp())>30').fetchall()

    # Insert new query history
    gp.gp_snowflake.execute("""
...
def log_run_event(details):
    ...
    insert = f'''
        ...
        where run_id=%s
    '''
    gp.gp_snowflake.execute(insert, args=(run_id, success))
