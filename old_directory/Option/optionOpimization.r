...
ATMExpectedSell$calls$ExpSellPrc <- ATMExpectedSell$calls$ExpSellPrc - ATMExpectedSell$calls[which.min(abs(ATMExpectedSell$calls$Strike - Stock_today)),'ExpSellPrc']
#TODO update using strike differences instead merge onto that.
# The at the money premium measurement
ATMExpectedSell$calls$Intrinsic <- round(Stock_today,0) - ATMExpectedSell$calls$Strike 

# will need to merge based on strike price
...
Option$calls$MktPrc <- Option$calls$MktPrc- Option$calls[Option$calls[,"Strike"]==TargetPrc,"MktPrc"]
