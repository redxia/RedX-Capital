...
# NOTE -- Changing the default source to 'kgrimm@guideponit.com' // dest to kgrimm@guidepoint.com
# default_src     = 'noreply@guidepointdata.com' #'jjones@guidepoint.com' #'no-reply@amazonses.com'
default_src     = 'kgrimm@guidepoint.com' #'jjones@guidepoint.com' #'no-reply@amazonses.com'
...
default_dest    = gp.settings.get('email.dest', default=['kgrimm@guidepoint.com'])
...
def send(subject, body="empty", body_filename=None, src=default_src, dest=default_dest, plain=False, attachments=[]):
    ...
    # NOTE -- Changing region to us-east-2 from us-east-1
    print('ESTABLISHING CONNECTION TO SES....\n')
    client = session.client('ses','us-east-2')
