...
# default_dest    = settings.gp2.get('email.dest', default=['jjones@guidepoint.com', 'spodder@guidepoint.com', 'lto@guidepoint.com', 'kdolgin@guidepoint.com'])
default_dest    = gp2.settings.get('email.dest', default=['data-insights-alerts@guidepoint.com'])
...
def send(subject, body="empty", body_filename=None, src=default_src, dest=default_dest, plain=False, attachments=[]):
    ...
    if body_filename != None and os.path.exists(body_filename):
        body = gp2.utils.read_file(body_filename)
